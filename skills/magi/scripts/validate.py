...
def load_agent_output(filepath: str) -> dict[str, Any]:
    try:
        ...
        if file_size > MAX_INPUT_FILE_SIZE:
            raise ValidationError("File too large.", filepath)
    ...
    if not isinstance(data, dict):
        raise ValidationError("Top-level JSON must be an object.", filepath)
    ...
    if not isinstance(confidence, (int, float)) or isinstance(confidence, bool):
        raise ValidationError("Confidence must be a number.", filepath)
    if not (0.0 <= confidence <= 1.0):
        raise ValidationError("Confidence must be between 0.0 and 1.0.", filepath)
    ...
    if not isinstance(findings, list):
        raise ValidationError("Findings must be a list.", filepath)
